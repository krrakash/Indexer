...
impl EthereumClientTrait for EthereumClient {
    ...
    async fn get_transfer_logs_for_block(&self, block_number: u64) -> IndexerResult<Vec<Log>> {
        ...
        loop {
            ...
            match self.provider.get_logs(&filter).await {
                ...
                Err(e) => {
                    if attempts >= max_attempts {
                        warn!("Failed to get logs for block {} after {} attempts: {}", block_number, max_attempts, e);
                        return Err(IndexerError::Ethereum(format!("Failed to get logs for block {}: {}", block_number, e)));
                    }
                    
                    warn!("RPC call failed for block {} (attempt {}/{}): {}. Retrying...", block_number, attempts, max_attempts, e);
                    
                    let delay = Duration::from_millis(200 * (1 << (attempts - 1)));
                    sleep(delay).await;
                }
            }
        }
    ...
    async fn scan_blocks(&self, start_block: u64, end_block: u64, batch_size: u64) -> IndexerResult<Vec<TransferEvent>> {
        ...
        while current_block <= end_block {
            ...
            info!("Scanning blocks {} to {} for transfer events", current_block, batch_end);
            
    
            for block_num in current_block..=batch_end {
                let logs = self.get_transfer_logs_for_block(block_num).await?;
                
                for log in logs {
                    if let Some(transfer_event) = TransferEvent::from_log(log, self.contract_address) {
                        all_events.push(transfer_event);
                    }
                }
                
        
                if block_num < batch_end {
    ...
    async fn subscribe_to_new_blocks(&self) -> IndexerResult<tokio::sync::mpsc::Receiver<u64>> {
        if let Some(ws_provider) = &self.ws_provider {
            let stream = ws_provider.subscribe_blocks().await
                .map_err(|e| IndexerError::Other(format!("Failed to subscribe to blocks: {}", e)))?;
            
            let (tx, rx) = tokio::sync::mpsc::channel(100);
            
    
            let block_numbers: Vec<u64> = stream
                .map(|block| block.number.unwrap_or_default().as_u64())
                .collect()
                .await;
            ...
            tokio::spawn(async move {
                for block_number in block_numbers {
                    let _ = tx_clone.send(block_number).await;
                }
            });
    ...
    async fn verify_block_finality(&self, block_number: u64, confirmations: u64) -> IndexerResult<bool> {
        ...
        let original_block = self.get_block(block_number).await?;
        let current_block = self.get_block(block_number).await?;
        
        match (original_block, current_block) {
            (Some(original), Some(current)) => {
                Ok(original.hash == current.hash)
            }
            _ => Ok(false),
        }
    }
